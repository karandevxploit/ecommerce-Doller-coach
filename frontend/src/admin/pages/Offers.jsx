...
const getOfferList = (responseData) => {
  if (Array.isArray(responseData)) return responseData;
  if (Array.isArray(responseData?.data)) return responseData.data;
  if (Array.isArray(responseData?.data?.offers)) return responseData.data.offers;
  if (Array.isArray(responseData?.data?.items)) return responseData.data.items;
  if (Array.isArray(responseData?.offers)) return responseData.offers;
...
export default function Offers() {
  ...
  const fetchOffers = async (signal) => {
    try {
      ...
      const res = await api.get("/admin/offers", { signal });
      const rawPayload = getOfferList(res?.data);

      setOffers(
        rawPayload.map((item, index) => {
          const mapped = mapOffer(item);

          return {
            ...mapped,
            raw: item,
            id: item?._id || item?.id || mapped?.id || `offer-${index}`,
          };
        })
      );
    } catch (err) {
      ...
      console.error("OFFERS_FETCH_ERROR:", err?.response?.data || err?.message);
      setOffers([]);
      toast.error(err?.response?.data?.message || "Marketing order sync failed");
    } finally {
      if (!signal?.aborted) {
        setLoading(false);
      }
  ...
  const openEdit = (offer) => {
    ...
    setFormData({
      ...
      discountValue: raw.discountValue ?? raw.discount ?? "",
      applyTo: raw.applyTo || "all",
      applyToCategory: raw.applyToCategory || "",
      applyToProductId: raw.applyToProductId || "",
      startDate: safeSplitDate(raw.startDate),
      endDate: safeSplitDate(raw.endDate),
  ...
  const validateForm = () => {
    const next = {};
    const discountValue = safeNumber(formData.discountValue, NaN);
    ...
    if (!Number.isFinite(discountValue) || discountValue <= 0) {
      next.discountValue = "Mathematical value required";
    }

    if (formData.discountType === "percentage" && discountValue > 100) {
      next.discountValue = "Percentage cannot be more than 100";
    }

    if (!formData.startDate || !formData.endDate) {
      next.endDate = "Temporal range required";
  ...
  const handleSubmit = async (e) => {
    ...
    if (savingRef.current || saving || !validateForm()) return;

    savingRef.current = true;
    setSaving(true);

    const discountValue = safeNumber(formData.discountValue);
    ...
    const payload = {
      ...
      applyToCategory:
        formData.applyTo === "category" ? formData.applyToCategory.trim() : "",
      applyToProductId:
        formData.applyTo === "product" ? formData.applyToProductId || null : null,
      startDate: formData.startDate,
      endDate: formData.endDate,
      usageLimit: safeNumber(formData.usageLimit),
      perUserLimit: safeNumber(formData.perUserLimit),
      isActive: Boolean(formData.isActive),
      image: formData.image,
      link: formData.link.trim(),
      description: formData.description.trim() || formData.title.trim(),
      minOrderAmount: safeNumber(formData.minOrderAmount),
      maxDiscount:
        formData.maxDiscount !== "" ? safeNumber(formData.maxDiscount) : null,
    };
    ...
    try {
      ...
      const message =
        err?.response?.data?.code === "DUPLICATE_REQUEST"
          ? "Save already in progress. Please wait."
          : err?.response?.data?.message || err?.message || "Save Failure";
      toast.error(message);
    } finally {
      window.setTimeout(() => {
    ...
    <div className="space-y-12">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-[#f1f5f9] p-3 rounded-xl border border-gray-100 shadow-inner">
        ...
        <div className="flex items-center gap-2 px-2">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest text-[#0f172a]">
            Update Monitor Live
          </span>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="py-24 text-center bg-[#f1f5f9] rounded-xl border border-dashed border-gray-200">
            <p className="text-xs font-black uppercase tracking-widest text-gray-300">
              No records found. No active marketing logics.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-gray-50">
            {offers.map((off) => {
              ...
              return (
                <div
                  key={off.id}
                  className={`flex items-center justify-between p-4 transition-all group border-b border-gray-50 last:border-0 ${isExpired
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      <p
                        className={`text-sm font-black uppercase tracking-tight transition-colors ${isExpired
                            ? "text-gray-400"
                            : "text-[#0f172a] group-hover:text-[#1e3a8a]"
                          }`}
                      >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                          <ShieldCheck size={10} className="text-[#1e3a8a]" />{" "}
                          {(off?.couponCode || "").toUpperCase()}
                        </div>
                        <div className="h-3 w-px bg-gray-200" />
                        <div
                          className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest ${isLimitReached ? "text-red-500" : "text-gray-400"
                            }`}
                        >
                          <Users size={10} /> {off.usedCount || 0} /{" "}
                          {off.usageLimit || "∞"} USED
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-100 bg-white overflow-y-auto animate-in fade-in duration-500">
          <div className="max-w-[1200px] mx-auto p-3 md:p-12">
            <div className="flex items-center justify-between mb-12 border-b border-gray-100 pb-6">
              ...
              <Button variant="icon" onClick={() => setFormOpen(false)}>
                <X size={20} />
              </Button>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-gray-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      />
                    </div>
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="h-11 w-11 shrink-0 flex items-center justify-center bg-gray-100 rounded-xl hover:bg-gray-200 text-gray-700 transition"
                        >
                          <RefreshCw size={16} />
                        </button>
                      </div>
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm space-y-4">
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateField("applyTo", e.target.value)}
                        className="w-full bg-gray-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      >
                  ...
                  {formData.applyTo === "category" && (
                    <div className="mt-4">
                      ...
                      <input
                        value={formData.applyToCategory}
                        onChange={(e) => updateField("applyToCategory", e.target.value)}
                        className="w-full bg-gray-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-gray-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50"
                        placeholder="unlimited"
                      />
                      {formData.discountType === "flat" && (
                        <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-tighter">
                          Only for percentage types
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm space-y-4">
                  ...
                  {formData.image ? (
                    <div className="relative aspect-video rounded-xl overflow-hidden border border-gray-200">
                      <img
                        src={resolveImageUrl(formData.image) || formData.image}
                        alt="Offer"
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 h-8 w-8 bg-white/90 backdrop-blur-sm rounded-lg flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition shadow-sm"
                      >
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-gray-600">
                        {uploading ? "Uploading..." : "Click to upload promotion image"}
                      </p>
                      <input
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-gray-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
