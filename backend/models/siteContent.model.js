 ...
 */
// siteContentSchema.index({ singleton: 1 }, { unique: true }); // Duplicated by definition
siteContentSchema.index({ "heroCarousel.order": 1 });
siteContentSchema.index({ isDeleted: 1 });
...
siteContentSchema.statics.getContent = async function () {
  const now = Date.now();
  if (cachedContent && now - lastFetch < CACHE_TTL) return cachedContent;
  ...
  try {
    ...
    logger.error("SITE_CONTENT_FETCH_FAILED", { error: err.message });
...
siteContentSchema.statics.updateContent = async function (updates, userId) {
  try {
    ...
    logger.error("SITE_CONTENT_UPDATE_FAILED", { error: err.message });
    throw err;
  }
};
